...
public class DropTargetingSession
{
    ...
    public bool TryEnter(UnitController unit, MapRoot mapRoot)
    {
        ...
        DropCommand.CollectDropTargetCoords(unit, root, _targetCoordsBuffer);
        ...
        SourceUnit = unit;
        HighlightManager.Instance?.ShowDropTargetHighlights(unit, clearExisting: true);
        return true;
    }
    ...
    public void Exit(bool clearHighlights)
    {
        ...
        if (clearHighlights)
            HighlightManager.Instance?.ClearDropHighlights();
    }
}
